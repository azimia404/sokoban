#include "player.h"
    ...
    void Player::move_player(int dx, int dy) {
        ...
        if (!LevelController::getInstance().is_cell_inside_level(next_row, next_column)) {
            return;
        }

        char &cell = LevelController::getInstance().get_level_cell(static_cast<size_t>(next_row), static_cast<size_t>(next_column));
        if (cell == FLOOR || cell == GOAL) {
            ...
            if (!LevelController::getInstance().is_cell_inside_level(target_row, target_column)) {
                return;
            }
            char &target_cell = LevelController::getInstance().get_level_cell(static_cast<size_t>(target_row), static_cast<size_t>(target_column));
            if (target_cell == FLOOR || target_cell == GOAL) {
                ...
                for (size_t row = 0; level_solved && row < level.rows; ++row) {
                    for (size_t column = 0; level_solved && column < level.columns; ++column) {
                        char cell_to_test = LevelController::getInstance().get_level_cell(row, column);
                ...
                if (level_solved) {
                    LevelController::getInstance().unload_level();
                    LevelController::getInstance().load_next_level();
